...
for (replication in 1:20){
  ...
  t1 = Sys.time()
  system(paste0("mplus ./Mplus/mixIRT_", MIRT_DIM, "D.inp"))
  ...
  fileConn<-file(paste0(c('./results/metrics/mplus_', MIRT_DIM, '_', replication, '.txt'), collapse=''))
  writeLines(c(as.character(acc), as.character(mse_itempars), as.character(msetheta), 
               as.character(var(itempars_est)), as.character(var(theta_est)), 
